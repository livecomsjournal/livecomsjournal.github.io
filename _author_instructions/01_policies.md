...
- When assigned to a journal issue, you will be notified by the editors about the `\pubvolume`, `\pubyear`, and `\articlenum` fields, recompile the PDF, and check into your GitHub repository under the folder `release/LiveCoMS_Article_VX.pdf`, where 'X' is the current article verrsion, along with any final corrections as noted above.
